...
def calculate_indoor_wbgt(tas, tasmax, hurs):
    ...
    tas_gpu = cp.asarray(tas)
    tasmax_gpu = cp.asarray(tasmax)
    hurs_gpu = cp.asarray(hurs)

    def calculate_wbt(T, RH):
        return T * cp.arctan(0.151977 * cp.sqrt(RH + 8.313659)) + cp.arctan(T + RH) - cp.arctan(RH - 1.676331) + 0.00391838 * (RH ** 1.5) * cp.arctan(0.023101 * RH) - 4.686035
    
    WBTm = calculate_wbt(tas_gpu, hurs_gpu)
    WBTmax = calculate_wbt(tasmax_gpu, hurs_gpu)
    
    WBGTmin_id = 0.7 * WBTm + 0.3 * tas_gpu
    WBGTmax_id = 0.7 * WBTmax + 0.3 * tasmax_gpu
    WBGThalf_id = (WBGTmax_id + WBGTmin_id) / 2
    
    # Transfer results back to CPU
    return cp.asnumpy(WBGTmin_id), cp.asnumpy(WBGTmax_id), cp.asnumpy(WBGThalf_id)
